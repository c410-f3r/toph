  ...
  fn build(self) -> GenesisConfig {
    GenesisConfig {
      ...
      pallet_aura: Some(AuraConfig {
        authorities: self.initial_authorities.iter().map(|x| (x.0.clone())).collect(),
      }),
      pallet_balances: Some(BalancesConfig {
        balances: self.endowed_accounts.iter().cloned().map(|k| (k, 1 << 60)).collect(),
      }),
      pallet_contracts: Some(pallet_contracts::GenesisConfig {
        current_schedule: pallet_contracts::Schedule::default(),
      }),
      pallet_grandpa: Some(GrandpaConfig {
        authorities: self.initial_authorities.iter().map(|x| (x.1.clone(), 1)).collect(),
      }),
...
fn get_from_seed<T: Public>(seed: &str) -> <T::Pair as Pair>::Public {
  T::Pair::from_string(&format!("//{}", seed), None).expect("static values are valid; qed").public()
}

fn properties() -> Properties {
  let mut properties = Map::new();
  properties.insert("tokenSymbol".into(), "TOPH".into());
